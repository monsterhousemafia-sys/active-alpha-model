...
from aa_decision_cockpit_readonly_snapshot import write_g0r4r2_review_snapshot
...
from review_submission_delivery import deliver_g0r4r2_outgoing_submission, submission_folder_rel

ROOT = _REPO_ROOT
G0R4R2_PHASE_ID = "G0R4R2_VERBATIM_AUTHORITATIVE_BASELINE_RESUBMISSION"
G0R4R2_ZIP = ROOT / "codex_g0r4r2_verbatim_authoritative_baseline_resubmission.zip"
G0R4R2_SHA = doc_path("codex_g0r4r2_verbatim_authoritative_baseline_resubmission.zip.sha256")
G0R4R2_ATTESTATION = ROOT / "codex_g0r4r2_detached_submission_attestation.json"
G0R4R2_VERIFY_REPORT = ROOT / "codex_g0r4r2_detached_package_verification_report.md"
INPUT_DIR_REL = "incoming_external_reviews/g0r4r2"
INPUT_DIR = ROOT / INPUT_DIR_REL
BASELINE_ORIGINALS_DIR = INPUT_DIR / "baseline_originals"
OUTGOING_DIR_REL = "outgoing_external_reviews/g0r4r2"
ALLOWED_BRANCHES = (
...
VERBATIM_INPUTS_ZIP = INPUT_DIR / "G0R4R2_REQUIRED_VERBATIM_AUTHORITY_BASELINE_INPUTS.zip"
APPROVAL_DOC_NAME = "EXTERNAL_REVIEW_APPROVAL_G0R4R2_REMEDIATION_RESUBMISSION_ONLY.md"
...
EXPECTED_APPROVAL_SHA256 = "296e20abd3d88ca9f6c7138b97ec2a19e9a51836406393067a8e6282b9d16af2"
EXPECTED_BUNDLE_SHA256 = "fe2a76e49bf6e6d385e0cf5666f2d74317ab68bfed0f473f2b8f1bb6bf98908f"
BUNDLE_ZIP_NAME = "G0R4R2_CODEX_INPUT_BUNDLE.zip"
BUNDLE_SIDECAR_NAME = f"{BUNDLE_ZIP_NAME}.sha256"
BUNDLE_MANIFEST_NAME = "G0R4R2_CODEX_INPUT_MANIFEST.json"
...
PAYLOAD_MANIFEST_NAME = "CODEX_G0R4R2_COMMITTED_PAYLOAD_MANIFEST.json"
G0R4R2_COMMIT_MSG = "fix: resubmit G0R4R2 with verbatim authoritative baseline"
...
PREEXISTING_PHASE_ARTIFACT_EXCLUSIONS: Tuple[str, ...] = (
    ...
    "docs/review/sidecars/codex_g0r4r2_verbatim_authoritative_baseline_resubmission.zip.sha256",
    ...
    "incoming_external_reviews/g0r4r2/G0R4R2_CODEX_INPUT_MANIFEST.json",
    "incoming_external_reviews/g0r4r2/G0R4R2_REQUIRED_VERBATIM_AUTHORITY_BASELINE_INPUTS.zip",
    "incoming_external_reviews/g0r4r2/EXTERNAL_REVIEW_APPROVAL_G0R4R2_REMEDIATION_RESUBMISSION_ONLY.md",
    "incoming_external_reviews/g0r4r2/EXTERNAL_REVIEW_APPROVAL_G0R4R2_REMEDIATION_RESUBMISSION_ONLY.md.sha256",
    "incoming_external_reviews/g0r4r2/EXTERNAL_REVIEW_DECISION_G0R4_REMEDIATION_REQUIRED.md",
    "incoming_external_reviews/g0r4r2/EXTERNAL_REVIEW_OBSERVED_HASH_G0R4.sha256",
    f"{submission_folder_rel('G0R4R2')}/",
    f"{OUTGOING_DIR_REL}/",
    "incoming_external_reviews/g0r4r2/baseline_originals/",
)
...
WORKTREE_DRIFT_PREFIX_EXCLUSIONS: Tuple[str, ...] = (
    ...
    f"{OUTGOING_DIR_REL}/",
    f"{submission_folder_rel('G0R4R2')}/",
    "docs/review/sidecars/codex_g0r4r_",
    "docs/review/sidecars/codex_g0r3_",
    "docs/review/sidecars/codex_g0r4_",
    "docs/review/sidecars/codex_g0r4_detached",
)
...
AUTHORIZED_G0R4R2_COMMIT_PATHS: Tuple[str, ...] = (
    ".gitattributes",
    "docs/phases/G0R4R2/CODEX_G0R4R2_PREFLIGHT.md",
    "docs/phases/G0R4R2/CODEX_G0R4R2_EXTERNAL_REJECTION_REMEDIATION_REPORT.md",
    "docs/integrity/session_logs/G0R4R2/CODEX_G0R4R2_GIT_STATUS.txt",
    "docs/phases/G0R4R2/CODEX_G0R4R2_COMMITTED_PAYLOAD_MANIFEST.json",
    "docs/phases/G0R4R2/CODEX_G0R4R2_INPUT_DISCOVERY_REPORT.md",
    "docs/phases/G0R4R2/CODEX_G0R4R2_EXTERNAL_INPUT_HASH_VERIFICATION.json",
    "docs/phases/G0R4R2/CODEX_G0R4R2_AUTHORIZATION_VERIFICATION.json",
    "docs/phases/G0R4R2/CODEX_G0R4R2_AUTHORITATIVE_BASELINE_VERBATIM_VERIFICATION.json",
    "docs/phases/G0R4R2/CODEX_G0R4R2_V5R_BASELINE_COMPARISON.json",
    "docs/integrity/protected_hashes/G0R4R2/CODEX_G0R4R2_PROTECTED_HASHES_BEFORE.json",
    "docs/integrity/protected_hashes/G0R4R2/CODEX_G0R4R2_PROTECTED_HASHES_AFTER.json",
    "docs/integrity/session_logs/G0R4R2/CODEX_G0R4R2_TEST_OUTPUT.txt",
...
def _path_excluded_from_drift(path: str, exclusions: Set[str]) -> bool:
    if path in exclusions:
        return True
    if path.startswith("G0R4R2_CURSOR_DROP_IN_PROJECT_ROOT"):
        return True
...
def write_input_discovery_report(
    *,
    ...
    lines = [
        "# CODEX G0R4R2 Input Discovery Report",
        "",
        f"Generated: {_utc_now()}",
        f"Working directory: {ROOT}",
        f"Branch: {branch}",
        f"HEAD: {head}",
        f"Phase: {G0R4R2_PHASE_ID}",
    ...
    _write_text(doc_path("CODEX_G0R4R2_INPUT_DISCOVERY_REPORT.md"), "\n".join(lines) + "\n")
...
def verify_input_bundle() -> Tuple[bool, Dict[str, Any]]:
    ...
    manifest_ok = (
        manifest_loaded
        and manifest.get("authorized_phase_only") == G0R4R2_PHASE_ID
...
def verify_external_remediation_approval() -> Tuple[bool, Dict[str, Any]]:
    ...
    authorized_phase_found = G0R4R2_PHASE_ID in body
    g1_authorized = bool(
...
def apply_g0r4r2_replacements() -> Tuple[bool, str]:
...
def build_zip_include_list() -> List[str]:
    ...
    docs = [
        doc_path("CODEX_G0R4R2_PREFLIGHT.md").relative_to(ROOT).as_posix(),
        doc_path("CODEX_G0R4R2_EXTERNAL_REJECTION_REMEDIATION_REPORT.md").relative_to(ROOT).as_posix(),
        doc_path("CODEX_G0R4R2_GIT_STATUS.txt").relative_to(ROOT).as_posix(),
        manifest_rel,
        doc_path("CODEX_G0R4R2_V5R_BASELINE_COMPARISON.json").relative_to(ROOT).as_posix(),
        doc_path("CODEX_G0R4R2_PROTECTED_HASHES_BEFORE.json").relative_to(ROOT).as_posix(),
        doc_path("CODEX_G0R4R2_PROTECTED_HASHES_AFTER.json").relative_to(ROOT).as_posix(),
        doc_path("CODEX_G0R4R2_INPUT_DISCOVERY_REPORT.md").relative_to(ROOT).as_posix(),
        doc_path("CODEX_G0R4R2_EXTERNAL_INPUT_HASH_VERIFICATION.json").relative_to(ROOT).as_posix(),
        doc_path("CODEX_G0R4R2_AUTHORIZATION_VERIFICATION.json").relative_to(ROOT).as_posix(),
        doc_path("CODEX_G0R4R2_AUTHORITATIVE_BASELINE_VERBATIM_VERIFICATION.json").relative_to(ROOT).as_posix(),
        doc_path("CODEX_G0R4R2_TEST_OUTPUT.txt").relative_to(ROOT).as_posix(),
        "G0R4R2-CHANGE_MANIFEST.json",
    ]
    snapshots = [
...
def build_exact_byte_zip(commit: str, include: List[str]) -> Tuple[str, List[str], Dict[str, bytes]]:
    if G0R4R2_ZIP.is_file():
        G0R4R2_ZIP.unlink()
    missing: List[str] = []
    zip_bytes: Dict[str, bytes] = {}
    with zipfile.ZipFile(G0R4R2_ZIP, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for rel in include:
            norm = _norm(rel)
            ...
            zip_bytes[norm] = blob
    return _sha256_file(G0R4R2_ZIP), missing, zip_bytes
    ...
    *,
    ...
    for entry in manifest.get("payload_entries") or []:
        ...
        if actual_zip != expected or actual_commit != expected or actual_zip != actual_commit:
            ...
            verified += 1
    sidecar_ok = False
    if G0R4R2_SHA.is_file():
        sidecar_ok = G0R4R2_SHA.read_text(encoding="utf-8").strip().split()[0] == zip_digest
...
def write_detached_attestation(
    *,
    ...
    payload = {
        ...
        "zip_file": G0R4R2_ZIP.name,
        "zip_sha256": zip_digest,
        "sidecar_file": G0R4R2_SHA.name,
...
def update_phase_catalog() -> None:
    ...
    if not any(p.get("phase_id") == G0R4R2_PHASE_ID for p in phases):
        phases.append(
            {
                "phase_id": G0R4R2_PHASE_ID,
                "phase_key": "G0R4R",
...
def update_review_registry() -> None:
    ...
    if not any(r.get("phase_id") == G0R4R2_PHASE_ID for r in reviews):
        reviews.append(
            {
                "phase_id": G0R4R2_PHASE_ID,
                "phase_key": "G0R4R",
...
def write_change_manifest(modified: List[str]) -> None:
    atomic_write_json(
        ROOT / "G0R4R2-CHANGE_MANIFEST.json",
        {
            "schema_version": 1,
            "phase": G0R4R2_PHASE_ID,
...
def write_git_status(*, branch: str, start_head: str) -> None:
    _write_text(
        doc_path("CODEX_G0R4R2_GIT_STATUS.txt"),
        "\n".join(
            [
                f"branch={branch}",
...
def write_preflight(start_head: str, branch: str) -> None:
    _write_text(
        doc_path("CODEX_G0R4R2_PREFLIGHT.md"),
        "\n".join(
            [
...
def write_report(*, test_rc: int, restoration_ok: bool) -> None:
    _write_text(
        doc_path("CODEX_G0R4R2_EXTERNAL_REJECTION_REMEDIATION_REPORT.md"),
        "\n".join(
            [
                ...
                "- Authorization basis: EXTERNAL_REVIEW_APPROVAL_G0R4R2_REMEDIATION_RESUBMISSION_ONLY.md",
                "- Preserve G0R4 exact-byte detached-attestation package structure",
                "- Non-self-referential CODEX_G0R4R2_COMMITTED_PAYLOAD_MANIFEST.json",
...
def write_g0r4r2_template() -> None:
    _write_text(
        ROOT / "EXTERNAL_REVIEW_APPROVAL_G0R4R2_TEMPLATE.md",
        "\n".join(
            [
    ...
    _write_text(
        ...
        "\n".join(
            [
                ...
                "G1: NOT AUTHORIZED",
                "",
                "Review ZIP: codex_g0r4r2_verbatim_authoritative_baseline_resubmission.zip",
                "",
                "Separately submit:",
...
def run_tests() -> Tuple[int, str]:
    ...
    _write_text(doc_path("CODEX_G0R4R2_TEST_OUTPUT.txt"), log)
...
def stage_allowlist_and_baseline(*, skip: Optional[Set[str]] = None) -> Tuple[bool, List[str], str]:
    ...
    authorized = set(AUTHORIZED_G0R4R2_COMMIT_PATHS) | set(baseline)
    ...
    for rel in list(AUTHORIZED_G0R4R2_COMMIT_PATHS) + baseline:
...
def commit_g0r4r2(include: List[str], head: str) -> Tuple[bool, str, List[str]]:
    manifest_rel = doc_path(PAYLOAD_MANIFEST_NAME).relative_to(ROOT).as_posix()
    ...
    proc = subprocess.run(
        ["git", "commit", "-m", G0R4R2_COMMIT_MSG],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=False,
...
def worktree_clean_for_packaging() -> bool:
    allowed = {
        G0R4R2_ZIP.name,
        G0R4R2_ATTESTATION.name,
        G0R4R2_VERIFY_REPORT.name,
        G0R4R2_SHA.relative_to(ROOT).as_posix(),
        G0R4R2_SHA.name,
        *PREEXISTING_PHASE_ARTIFACT_EXCLUSIONS,
    }
    packaging_only_modified = {"tools/complete_g0r4r2_submission.py"}
    for line in _run_git("status", "--porcelain").splitlines():
        ...
        if (
            path in allowed
            or path.endswith(G0R4R2_ZIP.name)
...
def _verify_final_output_filenames() -> Tuple[bool, Dict[str, bool]]:
    ...
    g0r4r2_only = all("g0r4r2" in name.lower() for name in names)
    old_g0r4r = any("codex_g0r4r_" in name and "g0r4r2" not in name.lower() for name in names)
    return g0r4r2_only and not old_g0r4r, {
...
def _deliver_review_submission_if_pass(*, ok: bool, verification: Dict[str, Any]) -> Optional[Path]:
    ...
    if G0R4R2_ATTESTATION.is_file():
        try:
            att = json.loads(G0R4R2_ATTESTATION.read_text(encoding="utf-8"))
            attestation_phase_ok = att.get("phase") == G0R4R2_PHASE_ID
        except json.JSONDecodeError:
            attestation_phase_ok = False
    if not filename_ok or not attestation_phase_ok:
            ...
            json.dumps(
                {
                    ...
                    "FINAL_ATTESTATION_PHASE_IS_G0R4R2": attestation_phase_ok,
                    **filename_flags,
                },
                indent=2,
            )
        ...
        return None
    folder = deliver_g0r4r2_outgoing_submission(
        root=ROOT,
        zip_path=G0R4R2_ZIP,
        sidecar_path=G0R4R2_SHA,
        attestation_path=G0R4R2_ATTESTATION,
        verify_report_path=G0R4R2_VERIFY_REPORT,
    )
...
def main() -> int:
    if _run_git("log", "-1", "--format=%s") == G0R4R2_COMMIT_MSG:
        ...
        if missing:
            print(json.dumps({"g0r4r2_status": "BLOCKED", "zip_missing": missing}, indent=2))
            return 1
        G0R4R2_SHA.parent.mkdir(parents=True, exist_ok=True)
        G0R4R2_SHA.write_text(f"{zip_digest}  {G0R4R2_ZIP.name}\n", encoding="utf-8")
        ...
        print(json.dumps({"g0r4r2_status": "PASS" if ok_pass else "BLOCKED", **verification}, indent=2))
    ...
    if not discovery["all_found"]:
        print(
            json.dumps(
                {
                    "g0r4r2_status": "BLOCKED",
                    "G0R4R2_LOCAL_REMEDIATION_STATUS": "BLOCKED",
    ...
    atomic_write_json(doc_path("CODEX_G0R4R2_EXTERNAL_INPUT_HASH_VERIFICATION.json"), bundle_payload)
    if not bundle_ok:
        print(
            json.dumps(
                {
    ...
    atomic_write_json(doc_path("CODEX_G0R4R2_AUTHORIZATION_VERIFICATION.json"), approval_payload)
    if not approval_ok:
        print(
            json.dumps(
                {
    ...
    ok_apply, apply_msg = apply_g0r4r2_replacements()
    if not ok_apply:
        print(
            json.dumps(
                {
    ...
    verbatim_ok, verbatim_payload, line_mismatch = verify_authoritative_baseline_verbatim()
    ...
    atomic_write_json(doc_path("CODEX_G0R4R2_PROTECTED_HASHES_BEFORE.json"), protected_hash_snapshot(v5r_paths))
    comparison, restoration_ok = build_comparison(include_set)
    atomic_write_json(
        doc_path("CODEX_G0R4R2_V5R_BASELINE_COMPARISON.json"),
        {
    ...
    atomic_write_json(doc_path("CODEX_G0R4R2_PROTECTED_HASHES_AFTER.json"), protected_hash_snapshot(v5r_paths))

    update_phase_catalog()
    update_review_registry()
    write_authorization_artifacts(ROOT)
    write_g0r4r2_review_snapshot(ROOT)
    write_change_manifest(list(AUTHORIZED_G0R4R2_COMMIT_PATHS))
    write_git_status(branch=branch, start_head=start_head)
    write_g0r4r2_template()
    update_next_cursor_prompt()

    test_rc, _ = run_tests()
    write_report(test_rc=test_rc, restoration_ok=restoration_ok)
    if test_rc != 0 or not restoration_ok:
        print(json.dumps({"g0r4r2_status": "BLOCKED", "blocker": "TESTS_OR_BASELINE"}, indent=2))
        return 1
    ...
    if not ok_drift:
        print(json.dumps({"g0r4r2_status": "BLOCKED", "unexpected": sorted(unexpected)}, indent=2))
        return 1

    ok, final_commit, _ = commit_g0r4r2(include, start_head)
    if not ok or final_commit == start_head:
        print(json.dumps({"g0r4r2_status": "BLOCKED", "blocker": "COMMIT_FAILED"}, indent=2))
        return 1

    if not worktree_clean_for_packaging():
        print(json.dumps({"g0r4r2_status": "BLOCKED", "blocker": "WORKTREE_NOT_CLEAN"}, indent=2))
        return 1
    ...
    zip_verbatim_ok, zip_verbatim_payload, zip_line_mismatch = verify_authoritative_baseline_verbatim(zip_bytes)
    ...
    print(
        json.dumps(
            {
                "g0r4r2_status": "PASS" if ok_pass else "BLOCKED",
