"""V2R cost stress and statistical validity remediation orchestration."""
...
V2R_PHASE = "V2R_COST_STRESS_AND_STATISTICAL_VALIDITY_REMEDIATION"
V2R_REVIEW_ZIP = "codex_v2r_statistical_validity_review.zip"
TEST_OUTPUT = "CODEX_V2R_TEST_OUTPUT.txt"
...
def reconcile_expected_next_for_v2r(root: Path) -> None:
    ...
    if executed != "V2_COST_STRESS_AND_ROBUSTNESS_ENGINE":
        return
    if state.get("execution_status") != "AWAITING_EXTERNAL_REVIEW":
        return
    allowed = allowed_next_phases(root, executed)
    if V2R_PHASE not in allowed:
        return
    if state.get("expected_next_phase") == V2R_PHASE:
        return
    new_state = dict(state)
    new_state["expected_next_phase"] = V2R_PHASE
...
def main() -> None:
    ...
    ts = utc_stamp()
    backup_dir = ROOT / "control" / "repair_backups" / f"{ts}_V2R"
    ...
    manifest_path.write_text(
        json.dumps({"created_at_utc": ts, "phase": V2R_PHASE, "files": manifest}, indent=2) + "\n",
        encoding="utf-8",
    )
    shutil.copy2(manifest_path, ROOT / "V2R-BACKUP_MANIFEST.json")

    before = {rel: sha256_file(ROOT / rel) for rel in PROTECTED}
    (doc_path("CODEX_V2R_PROTECTED_HASHES_BEFORE.json")).write_text(
        json.dumps(before, indent=2) + "\n", encoding="utf-8"
    )

    bootstrap_vision_automation(ROOT)
    reconcile_expected_next_for_v2r(ROOT)

    reg = register_external_approval(ROOT, phase_id=V2R_PHASE)
    if not reg.get("registered"):
        raise SystemExit(f"V2R authorization failed: {reg.get('errors')}")
    begin = begin_authorized_phase(ROOT, V2R_PHASE)
    if not begin.get("started"):
        raise SystemExit(f"V2R begin failed: {begin.get('errors')}")
    ...
    comp = complete_authorized_phase(ROOT, phase_id=V2R_PHASE, review_zip_name=V2R_REVIEW_ZIP)
    if not comp.get("completed"):
        raise SystemExit(f"V2R completion failed: {comp.get('errors')}")
    ...
    progress = {
        ...
        "current_phase": "V2R_EXTERNAL_REVIEW_REQUIRED",
        "authorized_phase": "",
        "completed_phases": [
    ...
    (doc_path("CODEX_V2R_PROTECTED_HASHES_AFTER.json")).write_text(
        json.dumps(after, indent=2) + "\n", encoding="utf-8"
    )
    state = load_automation_state(ROOT)
    print("V2R OK", ts, state.get("current_executed_phase"))
