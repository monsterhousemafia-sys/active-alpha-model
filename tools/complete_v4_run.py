"""V4 read-only Decision Cockpit GUI integration orchestration."""
...
V4_PHASE = "V4_DECISION_COCKPIT_GUI_INTEGRATION"
V4_REVIEW_ZIP = "codex_v4_gui_review.zip"
TEST_OUTPUT = "CODEX_V4_TEST_OUTPUT.txt"
...
def main() -> None:
    ...
    ts = utc_stamp()
    backup_dir = ROOT / "control" / "repair_backups" / f"{ts}_V4"
    ...
    manifest_path.write_text(
        json.dumps({"created_at_utc": ts, "phase": V4_PHASE, "files": manifest}, indent=2) + "\n",
        encoding="utf-8",
    )
    shutil.copy2(manifest_path, ROOT / "V4-BACKUP_MANIFEST.json")

    before = {rel: sha256_file(ROOT / rel) for rel in PROTECTED}
    (doc_path("CODEX_V4_PROTECTED_HASHES_BEFORE.json")).write_text(
        json.dumps(before, indent=2) + "\n", encoding="utf-8"
    )

    bootstrap_vision_automation(ROOT)

    reg = register_external_approval(ROOT, phase_id=V4_PHASE)
    if not reg.get("registered"):
        raise SystemExit(f"V4 authorization failed: {reg.get('errors')}")
    begin = begin_authorized_phase(ROOT, V4_PHASE)
    if not begin.get("started"):
        raise SystemExit(f"V4 begin failed: {begin.get('errors')}")
    ...
    comp = complete_authorized_phase(ROOT, phase_id=V4_PHASE, review_zip_name=V4_REVIEW_ZIP)
    if not comp.get("completed"):
        raise SystemExit(f"V4 completion failed: {comp.get('errors')}")
    ...
    progress = {
        ...
        "current_phase": "V4_EXTERNAL_REVIEW_REQUIRED",
        "authorized_phase": "",
        "completed_phases": [
    ...
    (doc_path("CODEX_V4_PROTECTED_HASHES_AFTER.json")).write_text(
        json.dumps(after, indent=2) + "\n", encoding="utf-8"
    )
    state = load_automation_state(ROOT)
    print("V4 OK", ts, state.get("current_executed_phase"))
