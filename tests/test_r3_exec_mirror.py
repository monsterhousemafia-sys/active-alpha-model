...
def test_mirror_page_renders_prognosis_section(tmp_path: Path) -> None:
    ...
    assert "r3-mirror-prognosis" in html
    assert "STX" in html
...
def test_mirror_page_results_and_exec_button(tmp_path: Path) -> None:
    ...
    assert "r3-mirror-exec-pkg" in html
    assert 'class="r3-label">Kauf</span>' in html
    assert 'class="r3-label">Verkauf</span>' in html
    assert "r3PollMirror" in html
    assert "Plan" in html
    assert "r3FreigabeSubmit" in html
    assert "r3PatchMirrorDisplays" in html
    assert "r3BindGraphNavigation" in html
    assert "STX" in html
    assert "SPY" in html
    ...
    assert "T212" in html
    assert 'class="r3-stock-btn' in html
    assert "Verkauf" in html
    assert "Kauf" in html
    assert "Uhr (CEST)" in html or "Uhr (CET)" in html
